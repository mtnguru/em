...
class WysiwygFilter extends FilterBase {
  ...
  public function tips($long = FALSE) {
    if ($long) {
      ...
      return t('Use [fn]...[/fn] (or &lt;fn&gt;...&lt;/fn&gt;) to insert automatically numbered az_wysiwyg.');
    }
  }
  ...
  public function process($text, $langcode) {
    ...
    $text = preg_replace('|\[az_wysiwyg([^\]]*)\]|', '<az_wysiwyg$1>', $text);
    ...
    // options to use.
    $this->replaceCallback($this->settings['az_wysiwyg_collapse'], 'prepare');
    ...
    // Replace tag <az_wysiwyg> with the list of az_wysiwyg.
    // If tag is not present, by default add the az_wysiwyg at the end.
   ...
   *
   * Uses static vars to temporarily store az_wysiwyg found.
  ...
  public function settingsForm(array $form, FormStateInterface $form_state) {
    $settings['az_wysiwyg_collapse'] = array(
      '#type' => 'checkbox',
      '#title' => t('Collapse az_wysiwyg with identical content'),
      '#default_value' => $this->settings['az_wysiwyg_collapse'],
      '#description' => t('If two az_wysiwyg have the exact same content, they will be collapsed into one as if using the same value="" attribute.'),
    );
    return $settings;
  }
   ...
   * list of az_wysiwyg it would be more efficient to maintain a separate array
   * with the footnote content as key, in order to do a hash lookup at this
   * stage. Since you typically only have a handful of az_wysiwyg, this simple
