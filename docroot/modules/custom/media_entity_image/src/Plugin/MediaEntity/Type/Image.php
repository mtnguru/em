  ...
  public function getField(MediaInterface $media, $name) {
    ...
    if (!empty($this->configuration['gather_exif']) && function_exists('exif_read_data')) {
      switch ($name) {
        ...
        case 'created':
          $date = $this->getExifField($uri, "GPSDateStamp");
          if ($date) {
            $time = $this->getExifField($uri, "GPSTimeStamp");
            $datetime = $date . " " . $time;

          } else {
            $datetime = $this->getExifField($uri, 'DateTimeOriginal');
            if (!$datetime) {
        ...
        case 'geolocation':
          $lat = $this->getExifField($uri, 'GPSLatitude');
          $lon = $this->getExifField($uri, 'GPSLongitude');
          return ($lat &&  $lon) ? 'POINT(' . $lon . ' ' . $lat . ')' : null;
//        return ($lat &&  $lon) ? ['lat' => $lat, 'lon' => $lon] : null;

        case 'file_size':
          return $this->getExifField($uri, 'FileSize');
      }
    }
