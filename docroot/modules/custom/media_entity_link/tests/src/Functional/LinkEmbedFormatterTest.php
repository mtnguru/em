...
namespace Drupal\Tests\media_entity_link\Functional;

use Drupal\Tests\media\Functional\MediaFunctionalTestBase;

/**
 * Tests for Link embed formatter.
 *
 * @group media_entity_link
 */
class LinkEmbedFormatterTest extends MediaFunctionalTestBase {
  ...
  /**
   * Tests adding and editing a link embed formatter.
   */
  public function testManageEmbedFormatter() {
    // Test and create one media type.
    $bundle = $this->createMediaType(['bundle' => 'link'], 'link');
    ...
    $this->assertFieldByName('source', 'link');
    ...
    $xpath = $this->xpath('//*[@id=:id]/td', [':id' => 'field-media-link']);
    $this->assertEqual((string) $xpath[0]->getText(), 'Link Url');
    $this->assertEqual((string) $xpath[1]->getText(), 'field_media_link');
    ...
    $edit = [
      'fields[field_media_link][parent]' => 'content',
      'fields[field_media_link][region]' => 'content',
      'fields[field_media_link][label]' => 'above',
      'fields[field_media_link][type]' => 'link_embed',
      'fields[field_embed_code][label]' => 'above',
      'fields[field_embed_code][type]' => 'link_embed',
    ];
    $this->drupalPostForm(NULL, $edit, t('Save'));
    $this->assertText('Your settings have been saved.');

    // Create and save the media with a link media code.
    $this->drupalGet('media/add/' . $bundle->id());

    // Random image url from link.
    $link_url = 'https://link.com/RamzyStinson/status/670650348319576064';

    // Random image from link.
    $link = '<blockquote class="link-link" lang="it"><p lang="en" dir="ltr">' .
             'Midnight project. I ain&#39;t got no oven. So I improvise making this milo crunchy kek batik. hahahaha ' .
             '<a href="https://link.com/hashtag/itssomething?src=hash">#itssomething</a> ' .
             '<a href="https://t.co/Nvn4Q1v2ae">pic.link.com/Nvn4Q1v2ae</a></p>&mdash; Zi (@RamzyStinson) ' .
             '<a href="https://link.com/RamzyStinson/status/670650348319576064">' .
             '28 Novembre 2015</a></blockquote><script async src="//platform.link.com/widgets.js" charset="utf-8"></script>';

    $edit = [
      'name[0][value]' => 'Title',
      'field_media_link[0][value]' => $link_url,
      'field_embed_code[0][value]' => $link,
    ];
    $this->drupalPostForm(NULL, $edit, t('Save'));
    ...
    $this->assertRaw('<a href="https://link.com/RamzyStinson/statuses/670650348319576064">', 'Link in embedded Link found.');

    // Assert that the string_long code formatter exists on this page.
    $this->assertText('Embed code');
    $this->assertRaw('<blockquote class="link-link', 'Embedded Link found.');
  }
