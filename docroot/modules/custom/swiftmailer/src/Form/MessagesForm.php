...
class MessagesForm extends ConfigFormBase {
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    $form['format']['css_inliner'] = array(
      '#type' => 'checkbox',
      '#title' => t('Inline CSS'),
      '#default_value' => $config->get('css_inliner'),
      '#description' => t('Convert CSS to inline styles to assure solid rendering in all email clients.'),
    );

    $form['image_mode'] = [
      '#type' => 'fieldset',
      '#title' => $this->t('Image paths'),
      '#description' => $this->t('Choose how to handle images. You can opt to convert image paths from relative to absolute,
        to embed images, or to leave image paths unchanged.'),
    ];

    $form['image_mode']['mode'] = [
      '#type' => 'radios',
      '#options' => [
        SWIFTMAILER_IMAGE_UNCHANGED => $this->t('Do not convert.'),
        SWIFTMAILER_IMAGE_ABSOLUTE => $this->t('Convert from relative to absolute paths.'),
        SWIFTMAILER_IMAGE_EMBED => $this->t('Embed images.')
      ],
      '#default_value' => $config->get('image_mode'),
    ];
