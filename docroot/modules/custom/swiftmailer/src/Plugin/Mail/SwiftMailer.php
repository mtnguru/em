...
class SwiftMailer implements MailInterface, ContainerFactoryPluginInterface {
   ...
   */
  protected $libraryDiscoverParser;
   ...
   * @param \Drupal\Core\Asset\LibraryDiscoveryParser $library_discover_parser
   */
  public function __construct(ImmutableConfig $transport, ImmutableConfig $message, LoggerInterface $logger, RendererInterface $renderer, ModuleHandlerInterface $module_handler, MailManagerInterface $mail_manager, LibraryDiscoveryParser $library_discover_parser, AssetResolverInterface $asset_resolver, FileSystem $file_system) {
  ...
  public function format(array $message) {
    ...
    if ($applicable_format == SWIFTMAILER_FORMAT_HTML) {
      ...
      if ($this->config['message']['css_inliner']) {
        $message['body'] = (new CssToInlineStyles())->convert($message['body'], $css);
      }
      else {
        // Insert the CSS into the header with a style element.
        $dom = new \DOMDocument();
        @$dom->loadHTML($message['body']);
        $head = $dom->getElementsByTagName('head')->item(0);
        if ($head == NULL) {
          $html = $dom->firstChild;
          $head = $html->insertBefore($dom->createElement('head'), $html->firstChild);
        }
        $style = $dom->createElement('style', $css);
        $type_attr = $dom->createAttribute('type');
        $type_attr->value = 'text/css';
        $style->appendChild($type_attr);
        $head->appendChild($style);
        $message['body'] = $dom->saveHTML();
      }
    }

    // Get how image paths should be handled.
    $image_mode = $this->config['message']['image_mode'];

    if ($image_mode == SWIFTMAILER_IMAGE_ABSOLUTE) {
      $message['body'] = Html::transformRootRelativeUrlsToAbsolute($message['body'], \Drupal::request()->getSchemeAndHttpHost());
    }

    /****************************************************************************
     * James Sorensen
     * Search for drupal-entity
     *   <drupal-entity
     *     data-align
     *     data-entity-embed-display="view_mode:media_medium"
     *     data-entity-uuid="adfasfsa">
     *   </drupal-entity>
     *
     * for each one found
     *   Load entity and replace string with <img src="asdf" class="az-medium" />
     ***************************************************************************/

    $pattern = "$\<drupal\-entity.*?\</drupal\-entity\>$";
    $embedded_media = [];
    preg_match_all($pattern, $message['body'], $embedded_media, PREG_OFFSET_CAPTURE);

//  $pattern = '$((data\-align)=\"(.*?)\"|(data\-entity\-embed\-display)=\"view_mode:(.*?))\"$';
    $pattern = '$((data\-align)=\"(.*?)\"|(data\-entity\-embed\-display)=\"view_mode:(.*?)\")+$';
//  $pattern = '$(data\-entity\-embed\-display)=\"view_mode:(.*?)\"$';
//  $pattern = '$(data\-align)=\"(.*?)\"$';
//  $pattern = '$data\-align$';
    for ($i = 0; $i < count($embedded_media[0]); $i++) {
      $media = $embedded_media[0][$i];

      $uuid = 'none';
      $pattern = '$data\-entity\-uuid=\"(.*?)\"$';      // data-align
      if (preg_match($pattern, $media[0], $matches)) {
        $uuid = $matches[1];
      };

      $align = 'none';
      $pattern = '$data\-align=\"(.*?)\"$';      // data-align
      if (preg_match($pattern, $media[0], $matches)) {
        $align = $matches[1];
      };

      $viewMode = 'default';
      $pattern = '$data\-entity\-embed\-display=\"view_mode:media\,(.*?)\"$';
      if (preg_match($pattern, $media[0], $matches)) {
        $viewMode = $matches[1];
      };

      $entity = \Drupal::entityManager()->loadEntityByUuid('media', $uuid);
      $file = \Drupal\file\Entity\File::load($entity->image[0]->target_id);
      $style = \Drupal::entityTypeManager()->getStorage('image_style')->load('thumbnail');
      $src = $style->buildUrl($file->getFileUri());
      $img = '<img src="' . $src . '" class="' . $viewMode . ' az-float-' . $align . '" />';
      $message['body'] = str_replace($media[0], $img, $message['body']);
    }
    ...
    // If the configured image mode calls for embedding inline images, search
    // the message body for filenames with .gif, .jpg and .png suffixes and
    // embed those too.
    if ($image_mode == SWIFTMAILER_IMAGE_EMBED) {
      $patterns += [
        '/src="(\/[^"]+\.gif)"/i',
        '/src="(\/[^"]+\.jpg)"/i',
        '/src="(\/[^"]+\.png)"/i',
    ...
    foreach ($patterns as $pattern) {
      $embeddable_images = [];
      preg_match_all($pattern, $message['body'], $embeddable_images);
      for ($i = 0; $i < count($embeddable_images[0]); $i++) {
        ...
        $message['body'] = str_replace($image_uri, 'cid:' . $image->cid, $message['body']);
